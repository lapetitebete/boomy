...
import __projectListing from "./__projectListing";
import {
...
import { DEMO_PROFESSIONALS } from "./professionals";
...
const DEMO_PROJECT_LISTINGS = __projectListing.map(
  (post, index): ProjectDataType => {
    //  ##########  GET CATEGORY BY CAT ID ######## //
    const category = DEMO_PROJECT_CATEGORIES.filter(
      (taxonomy) => taxonomy.id === post.listingCategoryId
    )[0];

    return {
      ...post,
      id: `projectListing_${index}_`,
      isAds: !index ? true : post.isAds,
      professional: DEMO_PROFESSIONALS.filter(
        (user) => user.id === post.professionalId
      )[0],
